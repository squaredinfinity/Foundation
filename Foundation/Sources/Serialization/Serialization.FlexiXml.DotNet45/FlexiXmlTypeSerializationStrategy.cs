...
namespace SquaredInfinity.Serialization.FlexiXml
{
        ...
        where T1 : class, ITypeSerializationStrategy<T1, T>
    {
        ...
        public virtual XElement Serialize(object instance, IFlexiXmlSerializationContext cx, string rootElementName, out bool hasAlreadyBeenSerialized)
        {
            ...
            var id = (InstanceId)null;

            if (instance != null && (instance.GetType().IsPrimitive || instance is string))
            {
                // nothing special needs to be done
                // string references will not be tracked
            }
            else
            {
                ...
                id =
                    ...
                    (_) =>
                    {
                        ...
                        var _id = new InstanceId(cx.GetNextUniqueId());
                        return _id;
                    });
            }
            ...
            out XObject serializedContent)
        {
            ...
            if (hasBeenSerializedBefore)
            {
                ...
                if(instanceIdRef.InstanceId == 49)
                {
