        ...
        public IEnumerable<ResourceDictionary> ImportAllResources(ICompositionService compositionService, bool merge)
        {
            ...
            using (var enumerator = all_providers.GetEnumerator())
            {
                while(enumerator.MoveNext())
                {
                    try
                    {
                        ResourcesDiagnostics.TraceSource.TraceInformation($"Importing xaml resources from {enumerator.Current.Value.GetType().FullName}...");
                    ...
                    {
                        ResourcesDiagnostics.TraceSource.TraceInformation(ex.ToString());
                    }
                }
            }
